...
private:
	...
	void validateType(ParameterType type) const;
public:
	Parameter() = default;
...
public:
	...
	template<typename... Tokens>
	void registerCommand(std::function<void(const Parameter*)> action, Tokens... tokens)
	{
		CommandBuilder cb;
		build_command(&cb, tokens...);
		addCommand(Command{ action, std::move(cb.tokens) });
	}

	void printCommands(std::ostream& o);
